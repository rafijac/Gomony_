...
def get_end_state():
    ...
    # Win: only one player's pieces remain
...
async def move_endpoint(body: MoveRequest):
    ...
    pre_end_state = get_end_state()
    if pre_end_state:
        # Return the end-state fields, do not process the move
        move_resp = {
            "valid": False,
        ...
        async def move_endpoint(body: MoveRequest):
            ...
            # Check for win/draw before processing move (handles board mutation edge case)
            pre_end_state = get_end_state()
            if pre_end_state:
                move_resp = {"valid": True, "reason": "Game already over", "board": board, "current_player": current_player, "pending_jump": pending}
                move_resp.update(pre_end_state)
